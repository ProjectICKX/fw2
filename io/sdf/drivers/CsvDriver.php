 ...
 * @varsion		2.0.0
 */
class CsvDriver extends abstracts\SdfDriverAbstract {
	...
	const DEFAULT_ENCODING_FILE		= \ickx\fw2\international\encoding\Encoding::SJIS_WIN;

	const DEFAULT_QUOTE_MODE		= self::QUOTE_CSV_AUTOMATIC;

	const DEFAULT_READ_LENGTH		= self::READ_LENGTH;

	const DEFAULT_EOL				= self::EOL_CRLF;

	const DEFAULT_DELIMITER			= self::DELIMITER_COMMA;

	const DEFAULT_ENCLOSURE			= self::ENCLOSURE_DOUBLE_QUOTE;

	const DEFAULT_ESCAPE			= self::ESCAPE_CHAR_SAME_ENCLOSURE;
	...
	public function read ($options = []) {
		$this->_pointer !== null ?: $this->readOpen();
		...
		if (is_callable($header_filter) && $this->_counter === 0) {
			$header = $header_filter(fgetcsv($this->_pointer, $this->_readLength, $this->_delimiter, $this->_enclosure, $this->_escape), $options);
			if ($header === false) {
				return [];
			}
		...
		if ($row_filter === null) {
			$this->_counter++;
			$row = fgetcsv($this->_pointer, $this->_readLength, $this->_delimiter, $this->_enclosure, $this->_escape);
			return $row;
		}

		while (($row = fgetcsv($this->_pointer, $this->_readLength, $this->_delimiter, $this->_enclosure, $this->_escape)) !== false) {
			$this->_counter++;
